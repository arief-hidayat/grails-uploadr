...
Uploadr.options = {
        ...
        onDelete: function(file, domObj) {
            var a = $.ajax( '<g:createLink plugin="uploadr" controller="upload" action="delete"/>',
                {
                    async: false,
                    headers: {
        ...
        onDownload: function(file, domObj) {
            ...
            // access to this file
            window.console && console.log('You clicked the \'download\' action for the following uploaded file:');
            window.console && console.log(file);
            window.console && console.log('in the following DOM element:');
            window.console && console.log(domObj);
            window.console && console.log('Implement a \'onDownload\' event handler to actually do something in the UI.');
            window.console && console.log('see: https://github.com/4np/grails-uploadr#event-handlers');
        },<g:if test="${classname != 'uploadr'}"> dropableClass: '${classname}-dropable', hoverClass: '${classname}-hover',</g:if>
<g:if test="${sound}"> notificationSound: '${resource(plugin: 'uploadr', dir:'sounds', file:'notify.wav')}',
